...
void setSpeedsvw(float v, float w)
{
  ...
  else
  {
    ...
    else if((w == 0.0f) && (v == 0.0f))               //edge case for 0
    {
  	  vL = 0.0f;
  	  vR = 0.0f;
    }
    else if((w == 0.0f) && (v != 0.0f))
      vL = vR = v;

    //cap wheel velocities
    vL = constrain(vL, -6.5f, 6.5f);
    vR = constrain(vR, -6.5f, 6.5f);
    
    //lcd.clear();
    lcd.setCursor(0,1);
    lcd.print("L,R: ");       //print to serial monitor vL and vR
    lcd.print(vL);
    lcd.print(", ");
    lcd.print(vR);
    setSpeedsIPS(vL, vR);
